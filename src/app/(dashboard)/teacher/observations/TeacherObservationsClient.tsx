...
export function TeacherObservationsClient({
  students,
  perStudent,
  recentObs,
}: {
  ...
  recentObs: ObservationWithLeaf[];
}) {
  const [search, setSearch] = useState("");
  const [activeTab, setActiveTab] = useState<"students" | "recent">("students");
  const studentMap = new Map(students.map((s) => [s.id, s]));

  const totalObs = Object.values(perStudent).reduce((a, r) => a + r.obsCount, 0);
    ...
    <div className="max-w-5xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredStudents.map((student) => {
              const rec = perStudent[student.id];
              const obsCount = rec?.obsCount ?? 0;
              const latest = rec?.latest ?? null;
              const currPct = rec?.currPct ?? 0;
              return (
                <Link key={student.id} href={`/teacher/observations/${student.id}`}>
                        ...
                        <div className="rounded-lg bg-slate-50 border border-slate-100 p-3 space-y-1.5">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <span className="text-[10px] text-slate-400">
                              {new Date(latest.created_at).toLocaleDateString("en-NG", {
                                day: "numeric",
                                month: "short",
                              })}
                            </span>
      ...
      {activeTab === "recent" && (
        <div className="space-y-3">
              ...
              <Card key={obs.id} className="border-slate-100 shadow-sm">
                <CardContent className="p-4">
                  <div className="flex items-start gap-3">
                    <div className={`w-9 h-9 rounded-full ${student?.avatar_color ?? "bg-slate-300"} text-white flex items-center justify-center text-xs font-bold shrink-0 mt-0.5`}>
                      {student ? initials(student.name) : "?"}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-1.5">
                        <Link
                          href={`/teacher/observations/${obs.student_id}`}
                        ...
                        {obs.leaf && (
                          ...
                          >
                            ...
                            {obs.leaf.leafName !== obs.leaf.activityName && (
                              <span className="opacity-60">{" · "}{obs.leaf.leafName}</span>
                            )}
                          </Badge>
                        )}
                        <span className="text-xs text-slate-400 flex items-center gap-1 ml-auto">
                          <Clock className="w-3 h-3" /> {formatDateTime(obs.created_at)}
                        </span>
                      </div>
          ...
          {recentObs.length === 0 && (
            <p className="text-sm text-slate-500 py-8 text-center">No observations logged yet.</p>
          )}
        </div>
      )}
    </div>
